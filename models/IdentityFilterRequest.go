...
type IdentityFilterRequest struct {
	...
	DateAuthenticated_EQ            primitive.DateTime `bson:"dateAuthenticated_EQ" json:"dateAuthenticated_EQ"`
	DateAuthenticated_GTE           primitive.DateTime `bson:"dateAuthenticated_GTE" json:"dateAuthenticated_GTE"`
	DateAuthenticated_LTE           primitive.DateTime `bson:"dateAuthenticated_LTE" json:"dateAuthenticated_LTE"`
...
func BuildIdentityFilterRequest(m map[string]interface{}, x *IdentityFilterRequest) {
	...
	if val, ok := m["dateAuthenticated_EQ"]; ok && val != nil {
		x.DateAuthenticated_EQ = val.(primitive.DateTime)
	}
	if val, ok := m["dateAuthenticated_GTE"]; ok && val != nil {
		x.DateAuthenticated_GTE = val.(primitive.DateTime)
	}
	if val, ok := m["dateAuthenticated_LTE"]; ok && val != nil {
		x.DateAuthenticated_LTE = val.(primitive.DateTime)
	}
