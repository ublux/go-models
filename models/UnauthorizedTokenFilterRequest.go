...
type UnauthorizedTokenFilterRequest struct {
	...
	ExpirationDate_EQ  time.Time `bson:"expirationDate_EQ" json:"expirationDate_EQ"`
	ExpirationDate_GTE time.Time `bson:"expirationDate_GTE" json:"expirationDate_GTE"`
	ExpirationDate_LTE time.Time `bson:"expirationDate_LTE" json:"expirationDate_LTE"`
...
func BuildUnauthorizedTokenFilterRequest(m map[string]interface{}, x *UnauthorizedTokenFilterRequest) {
	...
	if val, ok := m["expirationDate_EQ"]; ok && val != nil {
		x.ExpirationDate_EQ = val.(time.Time)
	}
	if val, ok := m["expirationDate_GTE"]; ok && val != nil {
		x.ExpirationDate_GTE = val.(time.Time)
	}
	if val, ok := m["expirationDate_LTE"]; ok && val != nil {
		x.ExpirationDate_LTE = val.(time.Time)
	}
